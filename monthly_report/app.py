...
.insight-box { background: #F0F4FF; border-radius: 8px; padding: 14px 18px; margin: 12px 0; border-left: 4px solid #1B4FD8; font-size: 13px; color: #374151; line-height: 1.8; }
.insight-box .risk { color: #DC2626; font-size: 12px; margin-top: 8px; }
.summary-box { background: rgba(27, 79, 216, 0.1); border-radius: 8px; padding: 16px 20px; margin: 12px 0; border-left: 4px solid #1B4FD8; font-size: 13px; color: #374151; line-height: 1.8; }
...
.author-card-h .rank { font-size: 14px; font-weight: 700; color: #1B4FD8; min-width: 28px; }
.author-card-h .name { font-weight: 600; color: #1B4FD8; min-width: 100px; }
.author-card-h .meta { color: #6B7280; font-size: 11px; flex: 1; }
.author-card-h .stat { text-align: right; font-weight: 600; color: #1B4FD8; min-width: 70px; }
...
.video-card-h .rank { font-size: 14px; font-weight: 700; color: #1B4FD8; min-width: 28px; }
.video-card-h .title { font-weight: 600; color: #1B4FD8; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.video-card-h .meta { color: #6B7280; font-size: 11px; min-width: 150px; }
.video-card-h .stat { font-weight: 600; color: #1B4FD8; min-width: 60px; text-align: right; }
...
.tier-card .name { font-weight: 600; color: #374151; min-width: 120px; }
.tier-card .value { font-weight: 700; color: #1B4FD8; min-width: 50px; text-align: center; }
.tier-card .pct { color: #6B7280; min-width: 50px; }
.tier-card .chg { min-width: 60px; text-align: right; }
...
.activity-stats { font-size: 12px; color: #374151; line-height: 1.8; }
...
.tier-card .name, .author-card-h .name, .video-card-h .title { font-weight: 600; color: #374151; min-width: 120px; }
.tier-card .value, .tier-card .pct { color: #1B4FD8; min-width: 50px; text-align: center; }
.activity-card { background: #F5F8FF; border-radius: 8px; padding: 14px 18px; border: 1px solid #E0E8FF; margin: 8px 0; }
.activity-name { font-size: 14px; font-weight: 600; color: #1B4FD8; margin-bottom: 10px; }
.todo-card { background: #F5F8FF; border-radius: 8px; padding: 14px 18px; border: 1px solid #E0E8FF; margin: 8px 0; }
.todo-title { font-size: 13px; font-weight: 600; color: #1B4FD8; margin-bottom: 8px; }
...
def render_kpi_cards(items: List[Dict], cols: int = 3) -> str:
    ...
    for item in items:
        ...
        cards.append(f'<div class="kpi-card"><div class="kpi-label">{label}</div><div class="kpi-value">{value}</div><div class="kpi-sub">{sub}</div></div>')
...
def render_tier_cards(rows: List[Dict]) -> str:
    ...
    for row in rows:
        ...
        cards.append(f'<div class="tier-card"><div class="name">{tier}</div><div class="value">{count}</div><div class="pct">{pct}</div><div class="chg">{chg}</div></div>')
    return ''.join(cards)
...
def render_author_card_h(row: Dict, rank: int) -> str:
    ...
    return f'<div class="author-card-h"><div class="rank">{rank_display}</div><div class="name"><a href="{author_link}" target="_blank">{author_name}</a></div><div class="meta">ID:{author_id} | 粉丝:{fans} | {platform}</div><div class="stat">{main_stat}</div></div>'
...
def render_video_card_h(row: Dict, rank: int) -> str:
    ...
    return f'<div class="video-card-h"><div class="rank">{rank_display}</div><div class="title"><a href="{video_link}" target="_blank">{title}</a></div><div class="meta">{author_name} | {platform}</div><div class="stat">播放:{plays}</div><div class="stat">赞:{likes}</div></div>'
...
def render_activity_card(name: str, data: Dict) -> str:
    ...
    return f'<div class="activity-card"><div class="activity-name">{name}</div><div class="activity-stats">投稿量：<b>{posts}</b> {change_html} ｜ 参与作者：<b>{authors}</b> ｜ 播放：<b>{plays}</b> ｜ CPM：<b>{cpm_html}</b></div></div>'
...
def create_pie_chart(data: List[Dict], title: str = ""):
    ...
    fig = go.Figure(data=[go.Pie(labels=labels, values=values, hole=0.3, textinfo='label+percent', textposition='outside')])
    fig.update_layout(title=title, margin=dict(l=20, r=20, t=30, b=20), height=320, showlegend=False)
    return fig

def create_line_chart(dates: List[str], values: List, title: str = "", events: List[Dict] = None, y_label: str = ""):
    fig = go.Figure()
    x_indices = list(range(len(dates)))
    fig.add_trace(go.Scatter(x=x_indices, y=values, mode='lines+markers', name=y_label, line=dict(color='#1B4FD8', width=2), marker=dict(size=6)))
    if events:
        for event in events:
            ...
            if event_date in dates:
                ...
                fig.add_vline(x=x_idx, line=dict(color='red', width=2, dash='dash'), annotation=dict(text=event_name, textangle=0, font=dict(size=10)))
...
def generate_html(data: Dict) -> str:
    ...
    if s1.get('enabled', True):
        parts.append('<div class="section-title">一、大盘概览</div>')
        parts.append(f'<div class="data-source">📊 数据来源：{s1.get("data_source", "-")}</div>')
    ...
    if s2.get('enabled', True):
        parts.append('<div class="section-title">二、创作者维度</div>')
        parts.append(f'<div class="data-source">📊 数据来源：{s2.get("data_source", "-")}</div>')
        if s2.get('insight_box'):
            parts.append(render_insight_box(s2['insight_box'].get('insight'), s2['insight_box'].get('risk')))
        if s2.get('kpi_cards', {}).get('items'):
            parts.append(render_kpi_cards(s2['kpi_cards']['items'], cols=4))
        tier = s2.get('tier_distribution', {})
        if tier.get('table'):
            thresholds = tier.get('thresholds', {})
            parts.append(f'<div class="subsection-title">粉段分层（{thresholds.get("tail_label", "<1万")} / {thresholds.get("mid_label", "1-30万")} / {thresholds.get("top_label", "≥30万")}）</div>')
            parts.append(render_tier_cards(tier['table']))
        lost = s2.get('lost_author_analysis', {})
        if lost:
            parts.append('<div class="subsection-title">流失作者归因分析</div>')
            parts.append(render_insight_box(lost.get('summary'), lost.get('risk')))
            if lost.get('cards'):
                parts.append(render_kpi_cards(lost['cards'], cols=3))
        alert = s2.get('activity_alert', {})
        if alert.get('cards'):
            parts.append('<div class="subsection-title">投稿活跃度预警</div>')
            parts.append(render_kpi_cards(alert['cards'], cols=3))
        for key, title_text in [('activity_down_top10', '投稿活跃度 降低 Top 10'), ('activity_up_top10', '投稿活跃度 增加 Top 10')]:
            table = s2.get(key, {})
            if table.get('rows'):
                parts.append(f'<div class="subsection-title">{title_text}</div>')
                for i, row in enumerate(table['rows'][:10], 1):
                    parts.append(render_author_card_h(row, row.get('rank', i)))
    
    # 板块三
    s3 = data.get('section_3_content', {})
    if s3.get('enabled', True):
        parts.append('<div class="section-title">三、内容维度</div>')
        parts.append(f'<div class="data-source">📊 数据来源：{s3.get("data_source", "-")}</div>')
        if s3.get('insight_box'):
            parts.append(render_insight_box(s3['insight_box'].get('insight'), s3['insight_box'].get('risk')))
        if s3.get('kpi_cards', {}).get('items'):
            parts.append(render_kpi_cards(s3['kpi_cards']['items'], cols=3))
        top3 = s3.get('viral_authors_top3', {})
        if top3.get('items'):
            parts.append('<div class="subsection-title">爆款作者 Top 3</div>')
            for item in top3['items']:
                parts.append(f'<div class="kpi-card" style="display:inline-block;margin:4px;min-width:150px;"><div style="font-size:14px;font-weight:600;color:#1B4FD8;">{item.get("rank_icon", "")} Top {item.get("rank", 1)}</div><div style="font-size:14px;font-weight:600;color:#1B4FD8;"><a href="{item.get("author_link", "#")}" target="_blank">{item.get("author_name", "-")}</a></div><div class="kpi-value">{item.get("viral_count", 0)}</div><div class="kpi-sub">条爆款 ｜ {item.get("author_id", "-")}</div></div>')
        videos = s3.get('viral_videos_top10', {})
        if videos.get('rows'):
            parts.append(f'<div class="subsection-title">爆款稿件 Top 10（≥{s3.get("viral_threshold", 50000)//10000}万播放）</div>')
            for i, row in enumerate(videos['rows'][:10], 1):
                parts.append(render_video_card_h(row, row.get('rank', i)))
        content_type = s3.get('content_type_analysis', {})
        if content_type.get('table'):
            parts.append('<div class="subsection-title">爆款内容类型归因</div>')
            if content_type.get('summary'):
                parts.append(render_insight_box(content_type['summary']))
            for t in content_type['table']:
                parts.append(f'<div class="tier-card"><div class="name">{t["type"]}</div><div class="value">{t["count_display"]}</div><div class="pct">{t["percentage"]}</div><div class="pct">条均:{t["avg_plays_display"]}</div></div>')
        avg_up = s3.get('avg_plays_up_top10', {})
        if avg_up.get('rows'):
            parts.append('<div class="subsection-title">条均播放增长 Top 10</div>')
            for i, row in enumerate(avg_up['rows'][:10], 1):
                parts.append(render_author_card_h(row, row.get('rank', i)))
        avg_down = s3.get('avg_plays_down_top10', {})
        if avg_down.get('rows'):
            parts.append('<div class="subsection-title">条均播放降低 Top 10</div>')
    ...
    if s4.get('enabled', True):
        parts.append('<div class="section-title">四、活动维度</div>')
        parts.append(f'<div class="data-source">📊 数据来源：{s4.get("data_source", "-")}</div>')
        if s4.get('insight_box'):
            parts.append(render_insight_box(s4['insight_box'].get('insight'), s4['insight_box'].get('risk')))
        main = s4.get('main_activity', {})
        if main.get('cur_period'):
            parts.append('<div class="subsection-title">主力活动对比</div>')
            cur = main['cur_period']
            parts.append(render_activity_card(f"当月：{cur.get('name', '-')}", cur))
            prev = main.get('prev_period', {})
            if prev:
                parts.append(render_activity_card(f"上月：{prev.get('name', '-')}", prev))
        sub = s4.get('sub_activities', {})
        if sub.get('rows'):
            parts.append('<div class="subsection-title">其他活动</div>')
            for row in sub['rows']:
                parts.append(render_activity_card(row.get('name', '-'), row))
        comm = s4.get('commissioned', {})
        if comm:
            parts.append('<div class="subsection-title">创作者约稿</div>')
            cur = comm.get('cur', {})
            if cur:
                parts.append(render_activity_card(f"当月（{cur.get('count', 0)}个活动）", cur))
            prev = comm.get('prev', {})
            if prev:
                parts.append(render_activity_card(f"上月（{prev.get('count', 0)}个活动）", prev))
    
    # 板块五
    s5 = data.get('section_5_livestream', {})
    if s5.get('enabled', False):
        parts.append('<div class="section-title">五、直播维度</div>')
        parts.append(f'<div class="data-source">📊 数据来源：{s5.get("data_source", "-")}</div>')
        if s5.get('insight_box'):
            parts.append(render_insight_box(s5['insight_box'].get('insight'), s5['insight_box'].get('risk')))
        if s5.get('kpi_cards', {}).get('items'):
            parts.append(render_kpi_cards(s5['kpi_cards']['items'], cols=4))
        tier = s5.get('tier_distribution', {})
        if tier.get('table'):
            parts.append('<div class="subsection-title">主播分层</div>')
            for t in tier['table']:
                parts.append(f'<div class="tier-card"><div class="name">{t["tier"]}</div><div class="value">{t["count"]}人</div><div class="pct">{t["pct"]}</div></div>')
        top = s5.get('top_streamers', {})
        if top.get('rows'):
            parts.append('<div class="subsection-title">直播作者 Top 10</div>')
    ...
    if s6.get('enabled', False):
        parts.append('<div class="section-title">六、核心作者分析</div>')
        parts.append(f'<div class="data-source">📊 数据来源：{s6.get("data_source", "-")}</div>')
        if s6.get('insight_box'):
            parts.append(render_insight_box(s6['insight_box'].get('insight'), s6['insight_box'].get('risk')))
        if s6.get('kpi_cards', {}).get('items'):
            parts.append(render_kpi_cards(s6['kpi_cards']['items'], cols=3))
        tier = s6.get('tier_distribution', {})
        if tier.get('table'):
            parts.append('<div class="subsection-title">作者分层</div>')
            for t in tier['table']:
                parts.append(f'<div class="tier-card"><div class="name">{t["tier"]}</div><div class="value">{t["count"]}人</div><div class="pct">{t["pct"]}</div></div>')
    ...
    if s7.get('enabled', True):
        parts.append('<div class="section-title">七、下月重点计划</div>')
        categories = s7.get('categories', [])
        if categories:
            for cat in categories:
                priority_color = {'高': '#DC2626', '中': '#F59E0B', '低': '#6B7280'}.get(cat.get('priority', '中'), '#374151')
                items = '<br>'.join(['• ' + item for item in cat.get('items', [])])
                parts.append(f'<div class="todo-card"><div class="todo-title" style="color:{priority_color};">{cat.get("name", "-")}（{cat.get("priority", "中")}优先级）</div><div class="todo-item">{items}</div></div>')
    
    parts.append(f'<div class="footer">创作者运营月报系统 v2.6 ｜ {datetime.now().strftime("%Y-%m-%d %H:%M")}</div>')
...
# =============================================================================
EXAMPLE_DATA = {
    "report_meta": {"product": "**", "month": "2026-02", "prev_month": "2026-01", "generate_time": "2026-03-05 10:30", "stat_period": "2026-02-01 ~ 2026-02-28"},
    "section_0_header": {"title": "** · 2026年2月 · 小喇叭创作者运营月报", "data_source": "内容营销系统 & 大盘看板", "summary": "2月大盘活跃创作者1,979人，环比下降15.3%，投稿供给有所收缩。内容消费方面，含三方播放2.45亿，环比小幅下降4.5%，互动量逆势上涨13.4%，内容共鸣有所提升。活动侧，主力激励活动参与1,944人，CPM 4.49元/千次，略高于3元参考值，需关注投入产出效率。内容方面，爆款777条（≥5万播放），联动赛季节点带动优质内容爆发。"},
    "section_1_dashboard": {
        "enabled": True,
        "data_source": "EDT大盘看板（用户提供）",
        "insight_box": {"insight": "2月大盘供给与消费双降，活跃作者环比减少357人（-15.3%），投稿量降至26,551条（-20.5%）。互动量逆势上涨13.4%，显示内容质量有所提升。", "risk": "活跃作者和投稿量同步下滑，供给端收缩明显，需关注创作者活跃度下降趋势。"},
        "kpi_cards": {"items": [
        ...
        "daily_trend": {"events": [{"date": "02-10", "name": "新赛季开启"}, {"date": "02-17", "name": "联动活动上线"}]},
        "by_platform": {"table": [
    ...
    "section_2_creators": {
        ...
        "insight_box": {"insight": "2月留存299人，但腰部作者净减少33人（196→163），部分有影响力的创作者活跃度下降明显。", "risk": "腰部作者（1-30万粉）从196人降至163人，降幅16.8%。"},
        "kpi_cards": {"items": [
        ...
        "tier_distribution": {"thresholds": {"top": 300000, "mid": 10000, "top_label": "≥30万", "mid_label": "1-30万", "tail_label": "<1万"}, "table": [
        ...
        "lost_author_analysis": {"summary": "流失105人中，尾部（<1万粉）57人（54.3%），腰部48人（45.7%），头部0人。", "risk": "腰部作者流失比例较高，建议针对腰部流失作者开展1v1触达。", "cards": [
    ...
    "section_3_content": {
        ...
        "insight_box": {"insight": "2月爆款777条（≥5万播放），较1月微增1.4%，爆款供给稳定。", "risk": "内容类型归因字段缺失，建议下月确保导出数据含内容类型标签。"},
        "kpi_cards": {"items": [
        ...
        "content_type_analysis": {"summary": "2月爆款内容以联动内容（48.8%）为主。", "table": [
        ...
        ], "pie_data": [{"label": "联动内容", "value": 379}, {"label": "车辆/涂装", "value": 178}, {"label": "游戏日常", "value": 166}]},
        "avg_plays_up_top10": {"rows": [
    ...
    "section_4_activities": {
        ...
        "insight_box": {"insight": "2月主力活动投稿25,972条，参与1,944人，CPM 4.49元，略高于参考值3元。", "risk": "CPM超参考值，需评估3月预算分配。"},
        "main_activity": {
            "cur_period": {"month": "2月", "name": "2月内容激励活动", "posts_display": "25,972", "posts_change": "▼18.8%", "authors_display": "1,944", "plays_display": "2.45亿", "cpm_display": "4.49元", "cpm_ref": "3.0元"},
            "prev_period": {"month": "1月", "name": "1月创作者内容激励活动", "posts_display": "31,983", "authors_display": "2,298", "plays_display": "1.09亿", "cpm_display": "10.10元"}
        },
        "sub_activities": {"rows": [
        ...
        "commissioned": {"cur": {"count": 3, "name": "创作者约稿", "posts_display": "456", "posts_change": "▲42.1%", "authors_display": "23", "plays_display": "89万", "cpm_display": "5.2元"}, "prev": {"count": 2, "name": "创作者约稿", "posts_display": "321", "authors_display": "18", "plays_display": "67万", "cpm_display": "4.8元"}}
    },
    "section_5_livestream": {
        "enabled": True,
        "data_source": "内容营销系统 - 直播数据",
        "insight_box": {"insight": "2月直播场次稳定，场均ACU表现良好。", "risk": "头部主播集中度较高，需关注长尾主播培养。"},
        "kpi_cards": {"items": [
        ...
        "tier_distribution": {"thresholds": {"top": 1000, "mid": 100}, "table": [
            {"tier": "头部（≥1000 ACU）", "count": 5, "pct": "11.1%"},
            {"tier": "腰部（100-1000 ACU）", "count": 18, "pct": "40.0%"},
            {"tier": "尾部（<100 ACU）", "count": 22, "pct": "48.9%"}
        ], "pie_data": [{"label": "头部", "value": 5}, {"label": "腰部", "value": 18}, {"label": "尾部", "value": 22}]},
        "top_streamers": {"rows": [
    ...
    "section_6_core_authors": {
        ...
        "insight_box": {"insight": "核心作者贡献稳定，活跃率维持较高水平。", "risk": "部分核心作者活跃度下降，需重点关注。"},
        "kpi_cards": {"items": [
    ...
    "section_7_todos": {
        "enabled": True,
        "categories": [
            {"name": "重点作者管理动作", "priority": "高", "items": ["Their**佬等腰部作者优先1v1触达", "流失腰部作者中粉丝≥5万的优先接触"]},
            {"name": "供给端运营动作", "priority": "中", "items": ["活跃作者下降明显，建议增加'复活奖励'", "快手、小红书占比极低，建议评估平台专项激励"]},
            {"name": "活动效率优化", "priority": "中", "items": ["CPM超参考值，建议引入内容质量评分机制"]}
        ]
    }
}
...
if not st.session_state.report_data:
    st.markdown('<div style="text-align:center;padding:60px 40px;"><h2 style="color:#1B4FD8;">📊 创作者运营月报系统</h2><p style="color:#6B7280;">请上传 JSON 文件或加载示例数据</p></div>', unsafe_allow_html=True)
    st.stop()
...
if s1.get('enabled', True):
    ...
    if by_platform.get('table'):
        ...
        for p in by_platform['table']:
            st.markdown(f'<div class="tier-card"><div class="name">{p["platform"]}</div><div class="value">{p["posts"]}</div><div class="pct">{p["posts_pct"]}</div><div class="pct">播放:{p["plays"]}</div><div class="pct">作者:{p["authors"]}</div></div>', unsafe_allow_html=True)
...
if s3.get('enabled', True):
    ...
    if top3.get('items'):
        ...
        for i, item in enumerate(top3['items']):
            with cols[i]:
                st.markdown(f'<div class="kpi-card"><div style="font-size:14px;font-weight:600;color:#1B4FD8;">{item.get("rank_icon", "")} Top {item.get("rank", i+1)}</div><div style="font-size:14px;font-weight:600;color:#1B4FD8;"><a href="{item.get("author_link", "#")}" target="_blank">{item.get("author_name", "-")}</a></div><div class="kpi-value">{item.get("viral_count", 0)}</div><div class="kpi-sub">条爆款 ｜ {item.get("author_id", "-")}</div></div>', unsafe_allow_html=True)
    ...
    if content_type.get('table'):
        ...
        for t in content_type['table']:
            st.markdown(f'<div class="tier-card"><div class="name">{t["type"]}</div><div class="value">{t["count_display"]}</div><div class="pct">{t["percentage"]}</div><div class="pct">条均:{t["avg_plays_display"]}</div></div>', unsafe_allow_html=True)
...
if s4.get('enabled', True):
    ...
    sub = s4.get('sub_activities', {})
    if sub.get('rows'):
        st.markdown('<div class="subsection-title">其他活动</div>', unsafe_allow_html=True)
        for row in sub['rows']:
...
if s5.get('enabled', False):
    ...
    if tier.get('table'):
        ...
        for t in tier['table']:
            st.markdown(f'<div class="tier-card"><div class="name">{t["tier"]}</div><div class="value">{t["count"]}人</div><div class="pct">{t["pct"]}</div></div>', unsafe_allow_html=True)
...
if s7.get('enabled', True):
    ...
    if categories:
        for cat in categories:
            priority_color = {'高': '#DC2626', '中': '#F59E0B', '低': '#6B7280'}.get(cat.get('priority', '中'), '#374151')
            items = '<br>'.join(['• ' + item for item in cat.get('items', [])])
            st.markdown(f'<div class="todo-card"><div class="todo-title" style="color:{priority_color};">{cat.get("name", "-")}（{cat.get("priority", "中")}优先级）</div><div class="todo-item">{items}</div></div>', unsafe_allow_html=True)
