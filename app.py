import streamlit as st
import pandas as pd
import plotly.graph_objects as go
import os

# --- 配置共享文件路径 ---
# 定义一个本地文件名，用于存储上传的数据，实现多用户间共享
SHARED_DATA_FILE = "shared_uploaded_data.xlsx"

# --- 页面基础配置 ---
st.set_page_config(page_title="数据趋势分析看板", layout="wide")
st.title("📊 播放与供给数据分析看板")
st.markdown("看板数据对所有用户共享。上传新文件后，页面将刷新并更新数据供所有人查看。")

# --- 核心功能函数 ---

def calculate_metrics(df, col_name):
    """
    计算增幅指标：
    1. 日环比增长量 (今天 - 昨天)
    2. 日环比增长率 ((今天 - 昨天) / 昨天)
    3. 7日移动平均 (用于平滑曲线)
    """
    df = df.copy()
    # 计算增长量
    df[f'{col_name}_增长量'] = df[col_name].diff()
    # 计算增长率，处理除以0的情况
    df[f'{col_name}_增长率'] = df[col_name].pct_change() * 100
    # 计算7日移动平均
    df[f'{col_name}_MA7'] = df[col_name].rolling(window=7).mean()
    
    # 填充空值为0，避免图表报错
    df.fillna(0, inplace=True)
    return df

def plot_dual_line_chart(df, x_col, y_col_1, y_col_2, title, yaxis_title):
    """绘制双线折线图"""
    fig = go.Figure()
    
    # 第一条线：原始数据
    fig.add_trace(go.Scatter(
        x=df[x_col], y=df[y_col_1],
        mode='lines', name=y_col_1,
        line=dict(color='#1f77b4', width=2)
    ))
    
    # 第二条线：MA7
    fig.add_trace(go.Scatter(
        x=df[x_col], y=df[y_col_2],
        mode='lines', name=y_col_2,
        line=dict(color='#ff7f0e', width=2, dash='dash') # 虚线区分
    ))
    
    fig.update_layout(
        title=title,
        xaxis_title="日期",
        yaxis_title=yaxis_title,
        hovermode="x unified",
        height=400,
        margin=dict(l=20, r=20, t=40, b=20)
    )
    return fig

def plot_growth_chart(df, x_col, val_col, rate_col, title):
    """
    绘制增幅混合图（柱状图+折线图）
    柱状图表示增长量，折线图表示增长率
    """
    fig = go.Figure()
    
    # 柱状图：增长量
    fig.add_trace(go.Bar(
        x=df[x_col], y=df[val_col],
        name='增长量',
        marker_color='#2ca02c',
        yaxis='y'
    ))
    
    # 折线图：增长率
    fig.add_trace(go.Scatter(
        x=df[x_col], y=df[rate_col],
        name='增长率(%)',
        mode='lines+markers',
        marker_color='#d62728',
        yaxis='y2' # 使用右侧坐标轴
    ))
    
    fig.update_layout(
        title=title,
        xaxis_title="日期",
        yaxis=dict(title="增长量", side="left"),
        yaxis2=dict(title="增长率 (%)", side="right", overlaying="y", showgrid=False),
        hovermode="x unified",
        height=400,
        margin=dict(l=20, r=20, t=40, b=20)
    )
    return fig

# --- 主程序逻辑 ---

# 1. 文件上传组件
uploaded_file = st.file_uploader("上传 Excel 数据文件（上传后将覆盖当前共享数据）", type=["xlsx", "xls"])

# 2. 处理上传逻辑：如果有新上传，保存到本地共享文件
if uploaded_file is not None:
    try:
        # 将上传的文件写入本地（覆盖旧文件）
        with open(SHARED_DATA_FILE, "wb") as f:
            f.write(uploaded_file.getbuffer())
        st.success("文件上传成功！数据已更新，所有用户刷新页面即可看到最新数据。")
    except Exception as e:
        st.error(f"保存共享文件时出错：{e}")

# 3. 读取并展示数据（优先读取共享文件）
if os.path.exists(SHARED_DATA_FILE):
    try:
        # 读取共享文件
        df = pd.read_excel(SHARED_DATA_FILE)
        
        # 简单的数据清洗：尝试自动识别日期列
        if '日期' not in df.columns:
            df.rename(columns={df.columns[0]: '日期'}, inplace=True)
        
        # --- 需求 2 修改：X轴 0 值替换逻辑 ---
        # 判断“日期”列是否为数值类型（如 0, 1, 2...）
        # pd.to_numeric 可以将数据转为数字，errors='coerce' 会将无法转换的变为 NaN
        temp_numeric = pd.to_numeric(df['日期'], errors='coerce')
        
        # 如果该列全是数字（不是时间格式），则进行特殊处理
        if temp_numeric.notna().all():
            # 先按数字大小排序，确保顺序正确 (0, 1, 2...)
            df['sort_key'] = temp_numeric
            df = df.sort_values('sort_key')
            df = df.drop(columns=['sort_key'])
            
            # 将 0 替换为文字。注意：需要转为字符串才能替换文字，否则图表可能报错
            df['日期'] = df['日期'].astype(str)
            df['日期'] = df['日期'].replace({'0': '专区开始当天', '0.0': '专区开始当天'})
        else:
            # 如果是标准日期格式，保持原有日期逻辑
            df['日期'] = pd.to_datetime(df['日期'])
            df = df.sort_values('日期')

        st.subheader("当前共享数据预览")
        st.dataframe(df.head())
        
        # 4. 数据计算
        # 检查必要的列是否存在
        required_cols = ['播放量', '供给量']
        missing_cols = [col for col in required_cols if col not in df.columns]
        
        if missing_cols:
            st.error(f"上传的文件中缺少以下列：{missing_cols}。请检查 Excel 表头是否包含'播放量'和'供给量'。")
        else:
            # 计算播放指标
            df = calculate_metrics(df, '播放量')
            # 计算供给指标
            df = calculate_metrics(df, '供给量')
            
            # 5. 展示图表 (竖向排列)
            st.markdown("---")
            
            # 卡片 1：播放趋势
            st.subheader("1. 播放量趋势")
            fig_play = plot_dual_line_chart(df, '日期', '播放量', '播放量_MA7', '播放量与7日均值', '播放量')
            st.plotly_chart(fig_play, use_container_width=True)
            
            # 卡片 2：播放增幅
            st.subheader("2. 播放量增幅分析")
            fig_play_growth = plot_growth_chart(df, '日期', '播放量_增长量', '播放量_增长率', '播放量日环比增幅')
            st.plotly_chart(fig_play_growth, use_container_width=True)
            
            # 卡片 3：供给趋势
            st.subheader("3. 供给量趋势")
            fig_supply = plot_dual_line_chart(df, '日期', '供给量', '供给量_MA7', '供给量与7日均值', '供给量')
            st.plotly_chart(fig_supply, use_container_width=True)
            
            # 卡片 4：供给增幅
            st.subheader("4. 供给量增幅分析")
            fig_supply_growth = plot_growth_chart(df, '日期', '供给量_增长量', '供给量_增长率', '供给量日环比增幅')
            st.plotly_chart(fig_supply_growth, use_container_width=True)
            
    except Exception as e:
        st.error(f"处理共享文件时出错：{e}")

else:
    st.info("等待文件上传... 目前服务器上没有共享数据。")
